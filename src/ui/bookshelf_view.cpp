...
void BookshelfView::setupUi() {
    setStyleSheet(
        ...
        "QLineEdit { background: #ffffff; border: 1.5px solid #dcc6ab; border-radius: 8px; padding: 6px 12px; color: #3b2212; font-size: 12px; }"
    ...
    lblBrand->setStyleSheet("font-size: 16px; font-weight: 900; color: #faebd7;");
    QLabel *lblSub = new QLabel("Children's Picture Book Studio", navBar);
    ...
    btnLoad->setStyleSheet("background: #fbf5ea; border: 1.5px solid #8d6b4f; border-radius: 6px; padding: 7px 14px; font-weight: bold; color: #3b2212;");
    ...
    btnCreate->setStyleSheet("background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #f3dfba, stop:1 #dfbe87); border: 1.5px solid #8c633f; border-radius: 6px; padding: 7px 16px; font-weight: 900; color: #3b2212;");
    ...
    contentLayout->setContentsMargins(48, 32, 48, 32);
    contentLayout->setSpacing(24);

    QHBoxLayout *heroCards = new QHBoxLayout();
    heroCards->setSpacing(16);

    auto createHeroCard = [this, heroCards](const QString &title, const QString &desc, const QString &actionText, const QString &color, auto clickHandler) {
        QPushButton *card = new QPushButton(this);
        card->setFixedHeight(110);
        card->setStyleSheet(
            "QPushButton { background: #fdfaf5; border: 2px solid #e2cfb9; border-radius: 12px; text-align: left; padding: 16px; } "
        ...
        t->setStyleSheet(QString("font-size: 15px; font-weight: 900; color: %1; background: transparent;").arg(color));
        QLabel *d = new QLabel(desc, card);
        d->setStyleSheet("font-size: 11px; color: #7a5c43; background: transparent;");
        QLabel *a = new QLabel(actionText, card);
        a->setStyleSheet("font-size: 11px; font-weight: 800; color: #8b5b32; background: transparent;");
    ...
    createHeroCard("Start a New Storybook", "Choose paper tint, corner flourishes, and author dedication.", "Open Wizard →", "#3b2212", [this]() {
        emit createProjectRequested();
    });

    createHeroCard("Load Existing Project", "Restore an illustrated notebook from a saved file on your disk.", "Browse Files →", "#588157", [this]() {
...
void BookshelfView::refreshGrid() {
    ...
    if (filtered.isEmpty()) {
        QLabel *empty = new QLabel("No story notebooks found matching your search.", m_gridContainer);
        empty->setStyleSheet("color: #7a5c43; font-size: 13px; font-style: italic; padding: 20px 0;");
    ...
    for (const auto &proj : filtered) {
        ...
        body->setContentsMargins(0, 14, 0, 14);
        body->setSpacing(4);

        QHBoxLayout *metaTop = new QHBoxLayout();
        QLabel *tBadge = new QLabel(theme.name, card);
        tBadge->setStyleSheet(QString("color: %1; font-weight: 800; font-size: 10px; border: 1px solid %2; border-radius: 4px; padding: 2px 6px;").arg(theme.frameColor.name(), theme.borderColor.name()));
        QLabel *dBadge = new QLabel(proj.createdAt, card);
        dBadge->setStyleSheet("color: #7a5c43; font-size: 10px;");
        ...
        lblTitle->setStyleSheet("font-size: 15px; font-weight: 900; color: #3b2212; border: none; margin-top: 4px;");
        body->addWidget(lblTitle);

        QLabel *lblAuthor = new QLabel(QString("by %1").arg(proj.author), card);
        lblAuthor->setStyleSheet("font-size: 11px; color: #7a5c43; border: none;");
        ...
        btnExport->setStyleSheet("background: transparent; border: 1px solid #dcc6ab; border-radius: 4px; padding: 3px 8px; font-size: 10px; font-weight: bold;");
        ...
        btnDup->setStyleSheet("background: transparent; border: 1px solid #dcc6ab; border-radius: 4px; padding: 3px 8px; font-size: 10px; font-weight: bold;");
        ...
        btnDel->setStyleSheet("background: transparent; border: 1px solid #fca5a5; border-radius: 4px; padding: 3px 6px; font-size: 10px; font-weight: bold; color: #b91c1c;");
        ...
        btnOpen->setStyleSheet("background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #f3dfba, stop:1 #dfbe87); border: 1px solid #8c633f; border-radius: 4px; padding: 3px 10px; font-size: 10px; font-weight: 900; color: #3b2212;");
